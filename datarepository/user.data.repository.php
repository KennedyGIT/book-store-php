...
class UserDataRepository {
    ...
    public function isUserCredentialsValid($username, $password) {
        $stmt = $this->conn->prepare("SELECT COUNT(*) AS count FROM Users WHERE Username = :username AND HashedPassword = :hashedPassword");
        $hashedPassword = hash('sha256', $password); // Assuming passwords are hashed in the database
        $stmt->bindParam(':username', $username);
